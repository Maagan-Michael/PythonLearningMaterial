...
`["Tony", "Dan", "Dvora"]`
#### Explanation
המתודה .append(object) מוסיף את האובייקט שהוכנס לסוף הרשימה כמו שרואים בדוגמא.  
המתודה .pop(int) מסיר את האובייקט שנמצע במיקום שהוכנס, יש אפשרות להשתמש עם .index() בשביל להסיר אובייקט ספציפי מהרשימה חשוב לזכור שיש להכניס רק מספרים.
